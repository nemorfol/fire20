...
<p>Nel tab "Coniuge" del profilo puoi attivare il <strong>partner</strong>: nome, anno di nascita, reddito lordo, tipo contratto (dipendente/autonomo/parasubordinato), pensione INPS attesa, contributo annuo al fondo pensione e il suo <strong>patrimonio investito attuale</strong> (che si somma al patrimonio del nucleo nel calcolo FIRE). Il calcolatore modella il nucleo a <strong>due redditi distinti</strong>: la fiscalita' italiana e' individuale (no joint filing americano), quindi due IRPEF separate, due INPS, eventualmente due fondi pensione. Le spese restano di nucleo (definite in <code>annualExpenses</code>).</p>
